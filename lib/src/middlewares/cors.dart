...
Middleware corsHeaders({
  ...
  return (Handler handler) {
    return (Request request) async {
      ...
      final innerHeaders = <String, List<String>>{
        ..._defaultHeadersAll,
        ...?headersAll,
      };

      final userProvidedAccessControlAllowOrigin = headers?[ACCESS_CONTROL_ALLOW_ORIGIN];

      if (userProvidedAccessControlAllowOrigin != null) {
        innerHeaders[ACCESS_CONTROL_ALLOW_ORIGIN] = [userProvidedAccessControlAllowOrigin];
        innerHeaders[VARY] = ['Origin'];
      } else {
        innerHeaders[ACCESS_CONTROL_ALLOW_ORIGIN] = [origin];
      }

      if (request.method == 'OPTIONS') {
        return Response(200, headers: innerHeaders);
      }
